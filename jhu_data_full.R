...
jhu_recovered <- as.data.frame(data.table::fread("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv"))
#jhu_deaths[is.na(jhu_deaths)]=0
total_recovered <- sum(jhu_recovered[,ncol(jhu_recovered)])
jhu_recovered = update_jhu(jhu_recovered, "recovered")
if (total_recovered!=sum(jhu_recovered[nrow(jhu_recovered),1:(ncol(jhu_recovered)-1)])) { 
  stop(paste0("Error: incorrect processing - total counts do not match"))
}
...
jhu_merge = merge(jhu_merge, jhu_recovered, by = "Date")
...
paste0("Error: mapping data lacking for the following countries: ",jhu_country_list[(jhu_country_list %in% countries$jhu_ID)==FALSE])
}
...
for (i in c(1:nrow(jhu_merge))) {
  ...
  jhu_subset_recovered = jhu_subset[,which(grepl("_recovered", names(jhu_subset)))]
  ...
  colnames(jhu_subset_recovered) = colnames(jhu_subset_recovered) %>% str_replace_all(., "_recovered", "") 
  ...
  for (j in 1:nrow(new_data)) {
    ...
    new_data$recovered[j] = jhu_subset_recovered[,country_name]
  }
...
collated_data = merge(collated_data, countries, by = "country")

cv_cases = collated_data
